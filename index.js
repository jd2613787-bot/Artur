...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        // ESTO ES LO NUEVO: CODIGO DE 8 DIGITOS
        if(!sock.authState.creds.registered){
            await new Promise(resolve => setTimeout(resolve, 3000))
            const code = await sock.requestPairingCode("54911153352003")
            console.log("\n\n==== TU CODIGO DE 8 DIGITOS ====")
            console.log(code)
            console.log("==== USA ESTE CODIGO EN WHATSAPP ====\n\n")
        }
